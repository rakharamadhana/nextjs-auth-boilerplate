export default async function Hero() {
    return (
        <section className="text-center mt-8 md:mt-12 px-4 md:px-8 lg:px-16">
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold text-indigo-700 dark:text-indigo-500">
                Welcome to the Mentoring Program!
            </h2>
            <p className="mt-4 text-base sm:text-lg md:text-xl text-primary max-w-2xl mx-auto">
                Learn, grow, and succeed with personalized mentorship tailored just for you.
            </p>
        </section>
    );
}
