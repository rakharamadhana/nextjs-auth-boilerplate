...
const MobileMenuToggle = () => {
    ...
    return (
        <div className="relative">
            ...
            {/* Mobile Menu */}
            <div
                className={`absolute top-12 right-0 bg-white shadow-lg transition-all duration-300 ease-in-out transform rounded-xl w-max ${isOpen ? 'scale-100 opacity-100' : 'scale-75 opacity-0'}`}
                style={{ transformOrigin: 'top right' }} // Set the origin for the scaling effect
            >
                <div className="flex flex-col space-y-2 py-2 px-4 max-w-max">
                    {isAdmin && (
                    ...
                    {session?.data && ( // Conditionally render User Profile link
